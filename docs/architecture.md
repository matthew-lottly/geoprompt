...
4. Frame methods expose reusable spatial analysis primitives such as nearest neighbors, map-window queries, radius queries, reprojection, spatial joins, proximity joins, dissolve, clip operations, overlay intersections, interaction tables, area similarity tables, and corridor accessibility.
...
- `frame.py` wraps point records with geometry-aware methods, CRS state, proximity and predicate join logic, and dissolve behavior.
...
- `overlay.py` isolates the Shapely-backed overlay adapters used for dissolve, clip, and intersection behavior.
