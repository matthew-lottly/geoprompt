...
Caption: mixed asset-demand pressure review generated by the package demo for a utility-style scenario.
Data source note: representative sample data from the repository fixtures, clearly synthetic but shaped to reflect operational screening workflows.
Process note: regenerated from the package demo and tracked in the figure manifest for release review.
...
- Extension guide: [docs/extending-geoprompt.md](docs/extending-geoprompt.md)
...
Choose GeoPrompt when the goal is not just geometry manipulation, but a full analyst-to-decision-support workflow: scenario comparison, resilience screening, routing, report generation, and stakeholder-ready outputs from the same package surface.
